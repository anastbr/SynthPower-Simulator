...
means <- matrixStats::rowMeans(beta_values_matrix, na.rm=TRUE)
means <- na.omit(means)
print(head(means))
return(means)
}
...
vector_of_affected_means <- ifelse(vector_of_affected_means>1, 1, vector_of_affected_means) # keep produced values within the range (0 and 1) specific to beta-value
...
```{r pressure, echo=FALSE}
# Generate beta values for every CpG across all samples. Due to the range of the real beta values (0 and 1), we "truncate"/adjust the values within the range. 
...
```{r pressure, echo=FALSE}
# Generate methylation data for every group based on the shape parameters specific to the group ( different means and common standard deviation). The output is data frame with all samples and all CpGs.
...
```{r pressure, echo=FALSE}
# Aggregated function which generates simulated data set for one environmental set-up (user-defined). The input is a vector with values for every parameter defining the environmental/experimental set-up. The output is the zip file with simulated data, indices of the truly modified CpGs, and input parameters of the user. 
...
# In this case we have used a data set with two groups (healthy versus patients with tuberculosis). Beta matrix data include Immumina array with 850 000 cpG probes. 
